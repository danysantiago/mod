...
routes.get("/products/whatshot", function (req, res, next) {
  var query = "SELECT *, IFNULL((SELECT SUM(rate)/COUNT(*) FROM seller_review WHERE reviewee_user_id = P.user_id), 0) as avg_seller_rating, (quantity - IFNULL((SELECT SUM(quantity) FROM order_detail OD WHERE P.product_id = OD.product_id), 0)) as stock FROM product P LEFT JOIN (SELECT product_id as pipd, min(product_image_id) as product_image_id, image_src FROM\n" + 
              " product_image GROUP BY product_id) PI ON pipd = P.product_id WHERE (IFNULL(auction_end_ts, NOW() + 1) > NOW()) HAVING (stock > 0) ORDER BY RAND() LIMIT 5;";
